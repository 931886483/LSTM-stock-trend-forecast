...
def LSTMtest(data, pianzhi, unit):
    ...
    def lstmCell():
        ...
        basicLstm = tf.nn.rnn_cell.BasicLSTMCell(rnn_unit, forget_bias=pianzhi, state_is_tuple=True)
        # dropout 未使用
    ...
    def prediction(time_step=20):
        ...
        with tf.Session() as sess:
            ...
            plt.savefig("./static/images/data2/"+str(pianzhi) + '-' + str(unit) + '-' +'loss.png')
            # 以折线图表示预测结果
            plt.figure()
            ...
            plt.savefig("./static/images/data2/"+str(pianzhi) + '-' + str(unit) + '-' + 'predict.png')
...
if __name__ == '__main__':
    ...
    # 单只股票数据处理
    df = pd.read_sql('select * from data2', con=conn)
    df_3_ = df.iloc[:, 3:].values
    LSTMtest(df_3_, 0.7, 2)
    LSTMtest(df_3_, 0.7, 14)
    LSTMtest(df_3_, 1.0, 2)
